...
# 💡 Твой Telegram ID — установлен корректно
ADMIN_ID = 7771113861  

# Хранилище данных пользователей в памяти
# Format: {user_id: {'gender': 'M'/'F', 'vip_until': timestamp, 'used_trial': True/False}}
users = {}

# Очереди поиска: queue[room][gender] -> list of user_ids
# gender keys: 'M', 'F'
queue = {
    'chat': {'M': [], 'F': []},
    'intim': {'M': [], 'F': []}
...
# Активные чаты {user_id: {'partner': partner_id, 'room': room}}
chats = {}

# Временное хранилище выбора параметров поиска {user_id: {'room': 'chat', 'target_gender': 'ALL'/'M'/'F'}}
search_prefs = {}
...
def handle_gender_set(call):
    ...
    bot.send_message(
        user_id,
        f"Твой пол успешно устанвлен: **{gender_str}**\n\nТеперь можно искать собеседника!",
        parse_mode="Markdown",
        reply_markup=get_main_keyboard(user_id)
    )

# Показ профиля и покупка VIP
...
def claim_trial(call):
    ...
    if user and not user['used_trial']:
        user['vip_until'] = time.time() + 86400  # 24 часа
...
def send_stars_invoice(call):
    ...
    prices = [types.LabeledPrice(label="VIP Статус на 30 дней", amount=150)] # 150 Stars
...
def process_successful_payment(message):
    ...
    # Выдаем VIP на 30 дней (2592000 сек)
...
def handle_room_selection(call):
    ...
    # Если есть VIP — даем выбрать пол, иначе ищем любого
    if is_vip(user_id):
...
def start_search(user_id, room, target_gender):
    ...
    # Ищем подходящего партнёра в очереди
    ...
    if partner_id:
        ...
        # Формируем сообщения с учетом VIP
        ...
        # Добавляем юзера в очередь своей комнаты по его полу
...
def find_partner(room, my_gender, target_gender):
    # Определение допустимых полов для поиска
    target_genders = ['M', 'F'] if target_gender == 'ALL' else [target_gender]

    # VIP пользователи всегда имеют приоритет (берем их первыми из очереди)
    # В этой реализации мы просто берем первого, кто ждет, 
    # так как разделение VIP/не VIP очереди усложнит код.
...
if __name__ == '__main__':
    print("Анонимный чат с VIP и Telegram Stars запущен!")
